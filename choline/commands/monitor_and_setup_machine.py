...
def check_for_choline_txt(vastai_id):
    print(f"Checking if machine {vastai_id} is up...", flush=True)

    # use show instance to get the proxy host/port (ssh-url returns direct IP which fails auth)
    ...
    try:
        ...
        d = _json.loads(info_result.stdout)
        host = d.get("ssh_host")
        port = str(d.get("ssh_port"))
        status = d.get("actual_status", "")
    except Exception:
        print(f"Machine {vastai_id}: could not parse instance info.", flush=True)
        return False
    ...
    result = subprocess.run(
        ["ssh", "-i", ssh_key,
         ...
         "-o", "ConnectTimeout=10",
         "-p", port, f"root@{host}", "ls ~/choline.txt"],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
    if result.returncode == 0 and "choline.txt" in result.stdout:
        print(f"Machine {vastai_id} is up!", flush=True)
        return True

    err = result.stderr.strip().splitlines()[-1] if result.stderr.strip() else "no output"
    print(f"Machine {vastai_id} not up yet. ({err})", flush=True)
    return False
...
def send_upload_locations(uhr_str, upload_locations, ignore_patterns, max_retries=100):
    ...
    # transfer custom_env.sh from ~/.choline to /root/.choline on remote
    ...
    if os.path.exists(custom_env):
        ...
        client.close()
        print("Transferred custom_env.sh to remote")
